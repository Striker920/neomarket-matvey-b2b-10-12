...
class TestSellerProductsList:
    ...
    def test_list_returns_only_own_products(self, client, db_session, valid_jwt_with_fixed_id):
        ...
        # Проверяем новые поля согласно b2b/openapi.yaml:1374-1384
    ...
    def test_deleted_products_hidden_by_default(self, client, db_session, valid_jwt_with_fixed_id):
        """Deleted products are hidden by default (include_deleted=False)"""
        token, seller_id = valid_jwt_with_fixed_id
    ...
    def test_deleted_products_visible_with_include_deleted(self, client, db_session, valid_jwt_with_fixed_id):
        """Deleted products are visible with include_deleted=true"""
        token, seller_id = valid_jwt_with_fixed_id
        ...
        # Проверяем, что удалённый товар имеет deleted=True
        deleted_item = next(item for item in data["items"] if item["title"] == "Deleted Product")
        assert deleted_item["deleted"] is True
        assert deleted_item["slug"] == "deleted"
        assert "category_id" in deleted_item
    ...
    def test_search_by_title_case_insensitive(self, client, db_session, valid_jwt_with_fixed_id):
        ...
        # Проверяем новые поля
        assert data["items"][0]["slug"] == "iphone-15-pro"
        assert "category_id" in data["items"][0]
    ...
    def test_response_includes_skus_count_and_total_active_quantity(self, client, db_session, valid_jwt_with_fixed_id):
        ...
        assert "category_id" in target_item
